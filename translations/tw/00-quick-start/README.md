...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T14:39:26+00:00",
...
此快速開始旨在讓你盡快使用 LangChain4j。它涵蓋了使用 LangChain4j 和 GitHub 模型構建 AI 應用的絕對基礎。在接下來的模組中，你將使用 Azure OpenAI 與 LangChain4j 建立更進階的應用。

## 什麼是 LangChain4j？

LangChain4j 是一個簡化構建 AI 驅動應用的 Java 函式庫。你不需要處理 HTTP 用戶端和 JSON 解析，而是使用乾淨的 Java API。

LangChain 中的「鏈」指的是將多個元件串接起來——你可能會將提示串接到模型，再串接到解析器，或將多個 AI 呼叫串接在一起，一個輸出作為下一個輸入。此快速開始專注於基礎，之後會探索更複雜的鏈。

<img src="../../../translated_images/langchain-concept.ad1fe6cf063515e1e961a13c73d45cfa305fd03d81bd11e5d34d0e36ed28a44d.tw.png" alt="LangChain4j Chaining Concept" width="800"/>

*LangChain4j 中的元件串接——建構塊連接以創造強大的 AI 工作流程*
...
**MessageWindowChatMemory** - 維護對話歷史。沒有它，每次請求都是獨立的。有了它，AI 會記住先前訊息並在多輪對話中保持上下文。

<img src="../../../translated_images/architecture.eedc993a1c57683951f20244f652fc7a9853f571eea835bc2b2828cf0dbf62d0.tw.png" alt="LangChain4j Architecture" width="800"/>

*LangChain4j 架構——核心元件協同運作，驅動你的 AI 應用*

## LangChain4j 依賴

此快速開始在 [`pom.xml`](../../../00-quick-start/pom.xml) 中使用兩個 Maven 依賴：
...
然後你可以在檔案總管中右鍵點擊任何範例檔（例如 `BasicChatDemo.java`），選擇 **「執行 Java」**，或使用「執行與除錯」面板中的啟動設定。
...
**使用 VS Code：** 只需在檔案總管中右鍵點擊任何範例檔，選擇 **「執行 Java」**，或使用「執行與除錯」面板的啟動設定（確保你已先將令牌加入 `.env` 檔案）。
...
從這裡開始，了解 LangChain4j 的最簡單用法。你會建立一個 `OpenAiOfficialChatModel`，用 `.chat()` 傳送提示，並取得回應。這展示了基礎：如何用自訂端點和 API 金鑰初始化模型。理解此模式後，其他功能都建立在此基礎上。
...
既然你知道如何與模型對話，接著探索你對它說什麼。此範例使用相同模型設定，但展示四種不同的提示模式。嘗試零次示範提示以直接指令、少次示範提示以範例學習、思維鏈提示以揭示推理步驟，以及角色提示以設定上下文。你會看到相同模型根據提示方式產生截然不同的結果。
...
這是 LangChain4j 強大的地方。你會使用 `AiServices` 建立一個能呼叫你 Java 方法的 AI 助手。只需用 `@Tool("描述")` 註解方法，LangChain4j 會處理其餘部分——AI 會根據使用者提問自動決定何時使用每個工具。這展示了函數呼叫，是構建能採取行動而不僅是回答問題的 AI 的關鍵技術。
...
> - 「此簡單方法與使用向量嵌入檢索有何不同？」
> - 「我如何擴展以處理多個文件或更大的知識庫？」
> - 「結構化提示的最佳實踐是什麼，以確保 AI 僅使用提供的上下文？」

## 除錯

範例包含 `.logRequests(true)` 和 `.logResponses(true)`，可在主控台顯示 API 呼叫，有助於排查認證錯誤、速率限制或意外回應。生產環境中請移除這些標記以減少日誌噪音。
...
**原因：** PowerShell 將 `=` 解讀為變數賦值運算子，破壞 Maven 屬性語法。
