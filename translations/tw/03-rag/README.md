...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T16:51:06+00:00",
...
在前面的模組中，你學會了如何與 AI 進行對話並有效地結構化提示。但有一個根本限制：語言模型只能知道它在訓練期間學到的內容。它們無法回答關於你公司政策、專案文件或任何未被訓練過的資訊的問題。
...
> **🤖 使用 [GitHub Copilot](https://github.com/features/copilot) Chat 試試看：** 開啟 [`DocumentService.java`](../../../03-rag/src/main/java/com/example/langchain4j/rag/service/DocumentService.java) 並問：
...
每個分塊會被轉換成一個數值表示，稱為嵌入向量——本質上是一種數學指紋，捕捉文本的意義。相似的文本會產生相似的嵌入向量。
...
**語意搜尋** - [RagService.java](../../../03-rag/src/main/java/com/example/langchain4j/rag/service/RagService.java)

當你提出問題時，你的問題也會被轉成嵌入向量。系統會將你的問題嵌入向量與所有文件分塊的嵌入向量進行比較。它會找到意義最相近的分塊——不只是關鍵字匹配，而是真正的語意相似。
...
> **🤖 使用 [GitHub Copilot](https://github.com/features/copilot) Chat 試試看：** 開啟 [`RagService.java`](../../../03-rag/src/main/java/com/example/langchain4j/rag/service/RagService.java) 並問：
...
確保根目錄有 `.env` 檔案，包含 Azure 認證（在模組 01 建立）：
```bash
...
> **注意：** 如果你已經使用模組 01 的 `./start-all.sh` 啟動所有應用程式，本模組已在 8081 埠運行。你可以跳過以下啟動指令，直接前往 http://localhost:8081。

**選項 1：使用 Spring Boot Dashboard（推薦 VS Code 使用者）**

開發容器包含 Spring Boot Dashboard 擴充套件，提供視覺化介面管理所有 Spring Boot 應用程式。你可以在 VS Code 左側活動列找到（尋找 Spring Boot 圖示）。
...
先上傳文件——TXT 檔案最適合測試。本目錄提供一個 `sample-document.txt`，包含 LangChain4j 功能、RAG 實作與最佳實踐資訊，非常適合測試系統。
...
觀察相關分數如何根據問題與文件內容的匹配度變化。

## 關鍵概念

**分塊策略**

文件被拆分成 300 個 token 的分塊，重疊 30 個 token。這種平衡確保每個分塊有足夠上下文且足夠小，可以在提示中包含多個分塊。
...
系統只會檢索高於最低閾值的分塊以確保品質。

**記憶體儲存**

本模組使用記憶體儲存以簡化流程。重啟應用程式後，上傳的文件會遺失。生產系統會使用持久化向量資料庫，如 Qdrant 或 Azure AI Search。
