...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T21:49:13+00:00",
...
此目錄包含使用 Bicep 與 Azure Developer CLI (azd) 的 Azure 基礎架構即程式碼 (IaC)，用於部署 Azure OpenAI 資源。
...
**簡化的本地開發設定** - 僅部署 Azure OpenAI，所有應用程式皆在本機執行。
...
> **注意：** `{resourceToken}` 是由訂閱 ID、環境名稱與位置產生的唯一字串
...
此操作將建立：
- 含 GPT-5 與 text-embedding-3-small 的 Azure OpenAI 資源
- 輸出連線詳細資訊
...
### 成本估算
- Azure OpenAI：依照輸入與輸出 token 計費
- GPT-5：約每百萬 token 3-5 美元（請查詢最新價格）
...
**解決方案**： 
1. 嘗試不同區域 - 請參考 [更改 Azure 區域](../../../../01-introduction/infra) 章節設定區域
...
**解決方案**：
1. 檢查 Azure 入口網站指標中的 OpenAI token 使用與限制
2. 若達到限制，增加 TPM 容量
