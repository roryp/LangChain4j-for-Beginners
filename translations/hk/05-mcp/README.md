...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T17:41:24+00:00",
...
你已經建立了會話式 AI，精通提示工程，讓回應以文件為基礎，並創建了帶有工具的代理。但這些工具都是為你的特定應用程式量身打造的。如果你能讓你的 AI 存取一個任何人都能創建和分享的標準化工具生態系統呢？

Model Context Protocol (MCP) 正是提供了這樣的標準方式，讓 AI 應用程式能發現並使用外部工具。你不需要為每個資料來源或服務撰寫自訂整合，而是連接到以一致格式公開其功能的 MCP 伺服器。你的 AI 代理就能自動發現並使用這些工具。

<img src="../../../translated_images/mcp-comparison.9129a881ecf10ff5448d2fa21a61218777ceb8010ea0390dd43924b26df35f61.hk.png" alt="MCP Comparison" width="800"/>
...
<img src="../../../translated_images/mcp-architecture.b3156d787a4ceac9814b7cffade208d4b0d97203c22df8d8e5504d8238fa7065.hk.png" alt="MCP Architecture" width="800"/>
...
<img src="../../../translated_images/transport-mechanisms.2791ba7ee93cf020ed801b772b26ed69338e22739677aa017e0968f6538b09a2.hk.png" alt="Transport Mechanisms" width="800"/>
...
用於遠端伺服器。你的應用程式向網路上某處運行的伺服器發送 HTTP 請求。使用 Server-Sent Events 進行即時通訊。
...
> **⚠️ Docker 使用者：** 執行範例 3 前，請用 `docker ps` 確認 Docker Desktop 正在執行。若出現連線錯誤，請啟動 Docker Desktop 並等待約 30 秒完成初始化。

## 快速開始

**使用 VS Code：** 在檔案總管中右鍵點擊任一示範檔案，選擇 **「Run Java」**，或使用執行與除錯面板的啟動設定（請先將權杖加入 `.env` 檔案）。

**使用 Maven：** 也可以從命令列執行以下範例。
...
**使用 VS Code：** 右鍵點擊 `StreamableHttpDemo.java`，選擇 **「Run Java」**。
...
**使用 VS Code：** 右鍵點擊 `StdioTransportDemo.java`，選擇 **「Run Java」**。
...
**使用 VS Code：** 右鍵點擊 `GitRepositoryAnalyzer.java`，選擇 **「Run Java」**。
...
MCP 用戶端連接時會自動發現可用工具。你的 AI 代理會看到工具描述，並根據使用者請求決定使用哪些工具。
...
Model Context Protocol 是一個開放標準，擁有不斷成長的生態系統：
...
此標準化意味著為一個 AI 應用程式打造的工具，也能用於其他應用，創造共享的能力生態系。
...
- 不同任務的提示工程模式（模組 02）  
- 以 RAG 方式讓回應以文件為基礎（模組 03）  
- 使用自訂工具創建 AI 代理（模組 04）  
- 透過 MCP 整合標準化工具（模組 05）

你現在擁有建置生產級 AI 應用的基礎。你學到的概念不受特定框架或模型限制，是 AI 工程的基本模式。
...
`--%` 運算子告訴 PowerShell 將所有剩餘參數原樣傳遞給 Maven，不進行解析。
...
**解決方法**：
- **建議**：切換至 WSL 2 模式（Docker Desktop 設定 → General → 「Use the WSL 2 based engine」）
- **替代方案（Hyper-V）**：將專案目錄加入 Docker Desktop 設定 → Resources → File sharing，然後重新啟動 Docker Desktop
