...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2026-01-16T08:16:28+00:00",
...
> В этом репозитории содержатся переводы более чем на 50 языков, что значительно увеличивает размер загрузки. Чтобы клонировать без переводов, используйте sparse checkout:
> ```bash
...
**Новичок в LangChain4j?** Ознакомьтесь с [Глоссарием](docs/GLOSSARY.md) для определения основных терминов и концепций.
...
4. [RAG (генерация с дополнением извлечением)](03-rag/README.md) - Создание интеллектуальных систем на базе знаний
...
> **Примечание:** В этом обучении используются как GitHub Models, так и Azure OpenAI. Модуль [Быстрого старта](00-quick-start/README.md) использует GitHub Models (подписка Azure не требуется), а модули 1-5 — Azure OpenAI.


## Обучение с GitHub Copilot

Чтобы быстро начать кодить, откройте этот проект в GitHub Codespace или в вашей локальной IDE с предоставленным devcontainer. Devcontainer, используемый в этом курсе, предварительно настроен с GitHub Copilot для совместного программирования с AI.

Каждый пример кода содержит предлагаемые вопросы, которые вы можете задать GitHub Copilot, чтобы углубить понимание. Ищите подсказки 💡/🤖 в:

- **Заголовках файлов на Java** - вопросы, специфичные для каждого примера
- **README модулей** - предложения для изучения после примеров кода

**Как использовать:** Откройте любой файл с кодом и задайте Copilot предложенные вопросы. Copilot имеет полный контекст кода и может объяснять, расширять и предлагать альтернативы.
