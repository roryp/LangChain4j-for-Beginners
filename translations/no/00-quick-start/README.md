...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T15:01:14+00:00",
...
Denne raske starten er ment å få deg i gang med LangChain4j så raskt som mulig. Den dekker det absolutt grunnleggende for å bygge AI-applikasjoner med LangChain4j og GitHub Models. I de neste modulene vil du bruke Azure OpenAI med LangChain4j for å bygge mer avanserte applikasjoner.
...
**ChatLanguageModel** – Grensesnittet for AI-modellinteraksjoner. Kall `model.chat("prompt")` og få en respons som streng. Vi bruker `OpenAiOfficialChatModel` som fungerer med OpenAI-kompatible endepunkter som GitHub Models.

**AiServices** – Lager typesikre AI-tjenestegrensesnitt. Definer metoder, annoter dem med `@Tool`, og LangChain4j håndterer orkestreringen. AI-en kaller automatisk dine Java-metoder når det trengs.
...
Denne raske starten bruker to Maven-avhengigheter i [`pom.xml`](../../../00-quick-start/pom.xml):
...
Modulen `langchain4j-open-ai-official` gir klassen `OpenAiOfficialChatModel` som kobler til OpenAI-kompatible APIer. GitHub Models bruker samme API-format, så ingen spesiell adapter trengs – bare pek base-URL til `https://models.github.ai/inference`.
...
7. Kopier og lagre tokenet ditt – du vil ikke se det igjen
...
Deretter kan du enkelt høyreklikke på en hvilken som helst demo-fil (f.eks. `BasicChatDemo.java`) i Utforskeren og velge **"Run Java"** eller bruke oppstartskonfigurasjonene fra Kjør og Feilsøk-panelet.
...
**Bruke VS Code:** Høyreklikk på en demo-fil i Utforskeren og velg **"Run Java"**, eller bruk oppstartskonfigurasjonene fra Kjør og Feilsøk-panelet (pass på at du har lagt til tokenet i `.env`-filen først).
...
Still spørsmål om innholdet i `document.txt`.
...
Start her for å se LangChain4j på sitt enkleste. Du lager en `OpenAiOfficialChatModel`, sender en prompt med `.chat()`, og får tilbake et svar. Dette demonstrerer grunnlaget: hvordan initialisere modeller med egendefinerte endepunkter og API-nøkler. Når du forstår dette mønsteret, bygger alt annet på det.
...
> - "Hvordan legger jeg til streaming-responser i stedet for å vente på komplett svar?"

**Prompt Engineering** - [PromptEngineeringDemo.java](../../../00-quick-start/src/main/java/com/example/langchain4j/quickstart/PromptEngineeringDemo.java)

Nå som du vet hvordan du snakker med en modell, la oss utforske hva du sier til den. Denne demoen bruker samme modelloppsett, men viser fire forskjellige prompt-mønstre. Prøv zero-shot prompts for direkte instruksjoner, few-shot prompts som lærer fra eksempler, chain-of-thought prompts som avslører resonnementstrinn, og rollebaserte prompts som setter kontekst. Du vil se hvordan samme modell gir dramatisk forskjellige resultater basert på hvordan du formulerer forespørselen.
...
> - "Hvordan integrerer jeg en ekte API i stedet for dette kalkulator-eksemplet?"
...
> **Merk:** Denne enkle tilnærmingen laster hele dokumentet inn i prompten. For store filer (>10KB) overskrider du kontekstgrenser. Modul 03 dekker oppdeling og vektorsøk for produksjons-RAG-systemer.
...
> - "Hva er forskjellen mellom denne enkle tilnærmingen og bruk av vektor-embedding for henting?"
> - "Hvordan kan jeg skalere dette for å håndtere flere dokumenter eller større kunnskapsbaser?"
> - "Hva er beste praksis for å strukturere prompten for å sikre at AI kun bruker den oppgitte konteksten?"

## Feilsøking

Eksemplene inkluderer `.logRequests(true)` og `.logResponses(true)` for å vise API-kall i konsollen. Dette hjelper med å feilsøke autentiseringsfeil, ratebegrensninger eller uventede svar. Fjern disse flaggene i produksjon for å redusere loggstøy.
...
**Navigasjon:** [← Tilbake til hovedside](../README.md) | [Neste: Modul 01 - Introduksjon →](../01-introduction/README.md)
...
**Årsak:** Maven må laste ned alle prosjektavhengigheter (Spring Boot, LangChain4j-biblioteker, Azure SDK-er osv.) ved første bygging.

**Løsning:** Dette er normal oppførsel. Påfølgende bygginger går mye raskere da avhengigheter er bufret lokalt. Nedlastningstiden avhenger av nettverkshastigheten din.

### PowerShell Maven-kommando-syntaks

**Problem:** Maven-kommandoer feiler med feilmelding `Unknown lifecycle phase ".mainClass=..."`

**Årsak:** PowerShell tolker `=` som en variabeltilordning, noe som bryter Maven-egenskaps-syntaksen
...
**Problem:** AI-svar viser søppel-tegn (f.eks. `????` eller `â??`) i stedet for emojis i PowerShell
