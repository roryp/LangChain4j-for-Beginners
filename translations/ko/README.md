...
LangChain4j와 Azure OpenAI GPT-5를 사용하여 기본 채팅부터 AI 에이전트까지 AI 애플리케이션을 구축하는 강의입니다.
...
모듈을 완료한 후, LangChain4j 테스트 개념을 직접 확인하려면 [테스트 가이드](docs/TESTING.md)를 탐색하세요.

> **참고:** 이 교육은 GitHub 모델과 Azure OpenAI를 모두 사용합니다. [빠른 시작](00-quick-start/README.md) 모듈은 GitHub 모델을 사용하며(Azure 구독 불필요), 1~5 모듈은 Azure OpenAI를 사용합니다.


## GitHub Copilot과 함께 배우기

빠르게 코딩을 시작하려면 이 프로젝트를 GitHub Codespace 또는 제공된 devcontainer를 사용해 로컬 IDE에서 열어보세요. 이 과정에서 사용된 devcontainer에는 AI 페어 프로그래밍용 GitHub Copilot이 미리 구성되어 있습니다.

각 코드 예제에는 GitHub Copilot에게 질문하여 이해를 심화할 수 있는 추천 질문이 포함되어 있습니다. 💡/🤖 표시를 확인하세요:

- **Java 파일 헤더** - 각 예제별 특정 질문
- **모듈 README** - 코드 예제 후 탐색 질문

**사용법:** 코드 파일을 열고 추천된 질문을 Copilot에게 물어보세요. Copilot은 코드베이스 전체를 이해하고 있어 설명, 확장, 대안 제시가 가능합니다.

더 알아보고 싶나요? [AI 페어 프로그래밍을 위한 Copilot](https://aka.ms/GitHubCopilotAI)를 확인하세요.
...
AI 앱 개발에 대해 막히거나 질문이 있으면 다음에 참여하세요:
...
**면책 조항**:  
이 문서는 AI 번역 서비스 [Co-op Translator](https://github.com/Azure/co-op-translator)를 사용하여 번역되었습니다. 정확성을 위해 노력하고 있으나, 자동 번역에는 오류나 부정확한 부분이 있을 수 있음을 유의해 주시기 바랍니다. 원문은 해당 언어의 권위 있는 원본 문서를 기준으로 간주해야 합니다. 중요한 정보의 경우 전문 인간 번역가의 번역을 권장합니다. 본 번역 사용으로 인해 발생하는 오해나 잘못된 해석에 대해 당사는 책임을 지지 않습니다.
