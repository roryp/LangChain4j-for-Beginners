...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2026-01-16T09:01:41+00:00",
...
> ```
> これにより、このコースを完了するのに必要なものがすべて揃い、はるかに高速なダウンロードが可能になります。
<!-- CO-OP TRANSLATOR LANGUAGES TABLE END -->

# LangChain4j ビギナーズガイド

LangChain4jとAzure OpenAI GPT-5を使ったAIアプリケーション構築のためのコース。基本的なチャットからAIエージェントまで。

**LangChain4jが初めてですか？** 重要な用語や概念については[用語集](docs/GLOSSARY.md)をご覧ください。
...
4. [RAG（リトリーバル増強生成）](03-rag/README.md) - インテリジェントな知識ベースシステムを構築
5. [ツール](04-tools/README.md) - 外部ツールや簡単なアシスタントの統合
6. [MCP（モデルコンテキストプロトコル）](05-mcp/README.md) - MCPとエージェントモジュールの活用
---
...
3. デフォルト設定を使用 – これにより本コース用の開発コンテナが選択されます
4. **Codespaceを作成** をクリック
5. 環境が準備されるまで5〜10分待つ
6. すぐに[クイックスタート](./00-quick-start/README.md)に進んで始めましょう！

モジュール修了後、[テストガイド](docs/TESTING.md)でLangChain4jのテスト概念を実践的に学べます。

> **注意:** このトレーニングではGitHubモデルとAzure OpenAIの両方を使用します。[クイックスタート](00-quick-start/README.md)モジュールではGitHubモデル（Azureサブスクリプション不要）を使用し、モジュール1〜5ではAzure OpenAIを使用します。

## GitHub Copilotでの学習

すぐにコーディングを始めたい場合は、このプロジェクトをGitHub Codespaceか付属のdevcontainerでローカルIDEで開きましょう。本コースで使用するdevcontainerには、AIペアプログラミング用のGitHub Copilotが事前設定されています。

各コード例には、理解を深めるためにGitHub Copilotに尋ねられる質問が含まれています。💡/🤖 のプロンプトを探してください：

- **Javaファイルのヘッダー** - それぞれの例に特化した質問
- **モジュールのREADME** - コード例後の探求用プロンプト

**使用方法:** 任意のコードファイルを開き、提案された質問をCopilotに投げかけてください。コードベースの全コンテキストを持ち、説明や拡張、代替案の提案が可能です。
...
製品のフィードバックやビルド中のエラーがある場合は、次を訪問してください：
...
**免責事項**：  
本書類はAI翻訳サービス[Co-op Translator](https://github.com/Azure/co-op-translator)を使用して翻訳されています。正確さを期していますが、自動翻訳には誤りや不正確な部分が含まれる可能性があることをご承知おきください。原文はあくまで正式かつ権威ある情報源とみなしてください。重要な情報については、専門の人間による翻訳を推奨いたします。本翻訳の使用に起因するいかなる誤解や誤訳についても、一切の責任を負いかねます。
