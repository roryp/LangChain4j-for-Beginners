...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T17:40:40+00:00",
...
你已經建立了會話式 AI，精通提示語，讓回應根據文件紮根，並創建了帶有工具的代理。但所有這些工具都是為你的特定應用程式量身定制的。如果你能讓你的 AI 存取一個任何人都能創建和分享的標準化工具生態系統呢？

模型上下文協議（MCP）正是提供了這樣的標準方式，讓 AI 應用程式能發現並使用外部工具。你不必為每個資料來源或服務撰寫自訂整合，而是連接到以一致格式公開其功能的 MCP 伺服器。你的 AI 代理就能自動發現並使用這些工具。

<img src="../../../translated_images/mcp-comparison.9129a881ecf10ff5448d2fa21a61218777ceb8010ea0390dd43924b26df35f61.mo.png" alt="MCP Comparison" width="800"/>

*MCP 之前：複雜的點對點整合。MCP 之後：一個協議，無限可能。*

## 理解 MCP

MCP 解決了 AI 開發中的一個根本問題：每個整合都是自訂的。想存取 GitHub？自訂程式碼。想讀取檔案？自訂程式碼。想查詢資料庫？自訂程式碼。而且這些整合都無法與其他 AI 應用程式共用。

MCP 將此標準化。MCP 伺服器以清晰的描述和結構公開工具。任何 MCP 用戶端都能連接、發現可用工具並使用它們。一次建置，到處使用。

<img src="../../../translated_images/mcp-architecture.b3156d787a4ceac9814b7cffade208d4b0d97203c22df8d8e5504d8238fa7065.mo.png" alt="MCP Architecture" width="800"/>
...
MCP 採用客戶端-伺服器模型。伺服器提供工具——讀取檔案、查詢資料庫、呼叫 API。用戶端（你的 AI 應用程式）連接伺服器並使用其工具。

**工具發現**

當你的用戶端連接到 MCP 伺服器時，它會詢問「你有什麼工具？」伺服器回應可用工具清單，每個工具附有描述和參數結構。你的 AI 代理便可根據使用者請求決定使用哪些工具。

**傳輸機制**

MCP 定義兩種傳輸機制：HTTP 用於遠端伺服器，Stdio 用於本地程序（包括 Docker 容器）：

<img src="../../../translated_images/transport-mechanisms.2791ba7ee93cf020ed801b772b26ed69338e22739677aa017e0968f6538b09a2.mo.png" alt="Transport Mechanisms" width="800"/>

*MCP 傳輸機制：HTTP 用於遠端伺服器，Stdio 用於本地程序（包括 Docker 容器）*

**可串流 HTTP** - [StreamableHttpDemo.java](../../../05-mcp/src/main/java/com/example/langchain4j/mcp/StreamableHttpDemo.java)

用於遠端伺服器。你的應用程式向網路上某處運行的伺服器發送 HTTP 請求。使用伺服器推送事件（Server-Sent Events）進行即時通訊。
...
> **🤖 嘗試使用 [GitHub Copilot](https://github.com/features/copilot) 聊天：** 開啟 [`StreamableHttpDemo.java`](../../../05-mcp/src/main/java/com/example/langchain4j/mcp/StreamableHttpDemo.java) 並詢問：
> - 「MCP 與第 04 模組中的直接工具整合有何不同？」
> - 「使用 MCP 進行跨應用程式工具共享有哪些好處？」
> - 「如何處理與 MCP 伺服器的連線失敗或逾時？」

**Stdio** - [StdioTransportDemo.java](../../../05-mcp/src/main/java/com/example/langchain4j/mcp/StdioTransportDemo.java)

用於本地程序。你的應用程式以子程序方式啟動伺服器，並透過標準輸入/輸出通訊。適用於檔案系統存取或命令列工具。
...
> **🤖 嘗試使用 [GitHub Copilot](https://github.com/features/copilot) 聊天：** 開啟 [`GitRepositoryAnalyzer.java`](../../../05-mcp/src/main/java/com/example/langchain4j/mcp/GitRepositoryAnalyzer.java) 並詢問：
> - 「Docker 傳輸如何隔離 MCP 伺服器？有何好處？」
...
- GitHub 個人存取權杖已在 `.env` 檔案中設定（來自模組 00）

> **注意：** 如果尚未設定 GitHub 權杖，請參閱 [模組 00 - 快速開始](../00-quick-start/README.md) 取得說明。
...
應用程式會自動啟動檔案系統 MCP 伺服器並讀取本地檔案。注意子程序管理是如何為你處理的。
...
- **推薦方案：** 切換至 WSL 2 模式以自動檔案共享（設定 → 一般 → 啟用「使用基於 WSL 2 的引擎」）。
...
範例自動處理平台差異（Windows 與 Unix 命令差異、Docker 路徑轉換）。這對於跨不同環境的生產部署非常重要。
...
模型上下文協議是一個開放標準，擁有不斷成長的生態系統：

- 官方 MCP 伺服器，涵蓋常見任務（檔案系統、Git、資料庫）
- 社群貢獻的各種服務伺服器
- 標準化的工具描述與結構
- 跨框架相容（適用任何 MCP 用戶端）

此標準化意味著為一個 AI 應用程式建置的工具可用於其他應用，創造共享的能力生態系。
...
- 不同任務的提示工程模式（模組 02）
- 以 RAG 讓回應根據文件紮根（模組 03）
- 使用自訂工具創建 AI 代理（模組 04）
...
`--%` 運算子告訴 PowerShell 將所有後續參數原樣傳遞給 Maven，不進行解析。
...
**問題**：Git 倉庫分析器報告倉庫為空或找不到檔案

**原因**：卷掛載（`-v`）因檔案共享設定問題無法正常工作
