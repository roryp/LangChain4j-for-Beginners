...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-13T21:47:34+00:00",
  "source_file": "01-introduction/infra/README.md",
  "language_code": "mo"
}
-->
# LangChain4j Azure 基礎設施入門指南
...
- [更新基礎設施](../../../../01-introduction/infra)
- [清理](../../../../01-introduction/infra)
- [檔案結構](../../../../01-introduction/infra)
- [安全建議](../../../../01-introduction/infra)
- [其他資源](../../../../01-introduction/infra)

此目錄包含使用 Bicep 和 Azure Developer CLI (azd) 的 Azure 基礎設施即程式碼 (IaC)，用於部署 Azure OpenAI 資源。
...
**簡化本地開發設定** - 僅部署 Azure OpenAI，所有應用程式在本地執行。
...
> **注意：** `{resourceToken}` 是由訂閱 ID、環境名稱和地區生成的唯一字串
...
兩個指令皆會自動從 `azd up` 建立的根目錄 `.env` 檔案載入環境變數。
...
若要在不同地區部署，請編輯 `infra/main.bicep`：
...
- 在 `infra/core/ai/cognitiveservices.bicep` 設定較低容量（10K TPM 取代 20K）
- 不使用時刪除資源：`azd down`
...
**解決方案：**
- 選擇支援 GPT-5 的地區（例如 eastus、swedencentral）
...
**解決方案**： 
1. 嘗試不同地區 - 請參考 [更改 Azure 地區](../../../../01-introduction/infra) 章節設定地區
...
**解決方案**：
1. 檢查 Azure 入口網站中 OpenAI 代幣使用與限制狀態
2. 若達到限制，增加 TPM 容量
...
4. **限制存取權限** - 使用 Azure RBAC 控制資源存取者
...
遇到問題時：
1. 查看上方的 [故障排除章節](../../../../01-introduction/infra)
2. 檢查 Azure 入口網站中的 Azure OpenAI 服務健康狀態
3. 在本專案倉庫開啟 issue
